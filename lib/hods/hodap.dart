...
class ap extends StatefulWidget {
  ...
  ap({this.myvar});
  @override
  _apState createState() => _apState(myvar);
}
...
showNotification(Map<String, dynamic> msg) async {
  ...
  update(String token) {
    ...
    databaseReference.child('fcm-tokenap/${token}').set({"token": token});
    textValue = token;
    setState(() {});
  }
//-------
...
class _ListpageState extends State<Listpage> {
 ...
 async {
   ...
   QuerySnapshot qn= await firestore.collection('Applied Science').getDocuments();
   return qn.documents;  
